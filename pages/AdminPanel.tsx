...
const AdminPanel: React.FC<AdminPanelProps> = ({ pin, onLogout }) => {
  ...
  const saveCase = async () => {
    ...
    setLoading(true);
    await apiCall('upsert_case', editingCase);
    setEditingCase(null);
    await loadCases();
    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="px-6 pb-20 max-w-5xl mx-auto">
            ...
            {editingCase ? (
              <div className="bg-white rounded-2xl p-6 border border-neutral-200">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-neutral-400 font-sans mb-2">Título</label>
                    <input
                      value={editingCase.title}
                      onChange={(e) => setEditingCase({ ...editingCase, title: e.target.value })}
                      className="w-full border border-neutral-200 rounded-xl px-4 py-3 text-sm font-sans focus:outline-none focus:ring-2 focus:ring-black"
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-neutral-400 font-sans mb-2">Descrição</label>
                    <textarea
                      value={editingCase.description}
                      onChange={(e) => setEditingCase({ ...editingCase, description: e.target.value })}
                      className="w-full border border-neutral-200 rounded-xl px-4 py-3 text-sm font-sans focus:outline-none focus:ring-2 focus:ring-black min-h-[100px]"
                      placeholder="Descrição do case"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-neutral-200 rounded-xl px-4 py-3 text-sm font-sans focus:outline-none focus:ring-2 focus:ring-black"
                      placeholder="Cole a URL da imagem (use a aba Mídia para fazer upload)"
                    />
                    {editingCase.cover_url && (
                      <img src={editingCase.cover_url} alt="Preview" className="mt-3 rounded-xl max-h-40 object-cover" />
                    )}
                  </div>
                ...
                <div className="flex gap-3 mt-8">
                  <button
                    onClick={saveCase}
                    disabled={loading || !editingCase.title}
                    className="bg-black text-white px-8 py-3 rounded-full text-sm font-sans font-bold uppercase tracking-wider hover:scale-105 transition-all disabled:opacity-50"
                  >
                    {loading ? 'Salvando...' : 'Salvar'}
                  </button>
                  <button
                    onClick={() => setEditingCase(null)}
                    className="border border-neutral-200 px-8 py-3 rounded-full text-sm font-sans text-neutral-600 hover:bg-neutral-50 transition-all"
                  >
                    Cancelar
                  </button>
                </div>
              ...
              <>
                <button
                  ...
                  className="bg-black text-white px-8 py-3 rounded-full text-sm font-sans font-bold uppercase tracking-wider hover:scale-105 transition-all mb-6"
                >
                  + Novo Case
                </button>
                {loading ? (
                  ...
                  <div className="space-y-4">
                    {cases.map((c) => (
                      <div key={c.id} className="bg-white rounded-2xl p-4 border border-neutral-200 flex items-center gap-4">
                        {c.cover_url && (
                          <img src={c.cover_url} alt={c.title} className="w-20 h-14 rounded-xl object-cover flex-shrink-0" />
                        )}
                        <div className="flex-grow min-w-0">
                          <h3 className="font-display text-lg truncate">{c.title}</h3>
                          <p className="text-xs text-neutral-400 font-sans truncate">{c.category}</p>
                        </div>
                        <div className="flex gap-2 flex-shrink-0">
                          {!c.is_visible && (
                            <span className="text-[10px] bg-neutral-100 text-neutral-400 px-2 py-1 rounded-full font-sans">Oculto</span>
                          )}
                          <button
                            onClick={() => setEditingCase(c)}
                            className="text-xs font-sans text-neutral-500 hover:text-black px-3 py-2 rounded-lg hover:bg-neutral-50 transition-all"
                          >
                            Editar
                          </button>
                          <button
                            onClick={() => c.id && deleteCase(c.id)}
                            className="text-xs font-sans text-red-400 hover:text-red-600 px-3 py-2 rounded-lg hover:bg-red-50 transition-all"
                          >
                            Excluir
                          </button>
                        </div>
        ...
        {tab === 'media' && (
          <div>
            <div className="mb-6">
              <label className="bg-black text-white px-8 py-3 rounded-full text-sm font-sans font-bold uppercase tracking-wider hover:scale-105 transition-all cursor-pointer inline-block">
                  ...
                  return (
                    <div key={name} className="bg-white rounded-2xl border border-neutral-200 overflow-hidden">
                      {isVideo ? (
                        ...
                        <img src={url} alt={name} className="w-full aspect-video object-cover" />
                      )}
                      <div className="p-3">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => copyUrl(url)}
                            className="text-[10px] font-sans bg-neutral-100 hover:bg-neutral-200 px-3 py-1.5 rounded-lg transition-all"
                          >
                            Copiar URL
                          </button>
                          <button
                            onClick={() => deleteMedia(name)}
                            className="text-[10px] font-sans text-red-400 hover:text-red-600 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-all"
                          >
                            Excluir
                          </button>
                        </div>
